...
try:
    ...
    print((Bcolors.BOLD +
           ...
           "'pip install git+https://github.com/CiscoDevNet/intersight-python.git'" +
           Bcolors.ENDC))
    exit()
...
class DriverInvReader(InvReader):
    ...
    def __init__(self, hostname, os_type, host_type, os_collection):
        InvReader.__init__(self)
        self.host = hostname
    ...
    def get_driver_inv(self):
        ...
        try:
            ...
            print((Bcolors.WARNING +
                   '[WARNING]: Issue while parsing kernel version' +
                   Bcolors.ENDC))
            print((Bcolors.BOLD + "[WARNING-DETAIL]: " + str(err) + Bcolors.ENDC))
...
class IntersightConnectionManager:
    ...
    def connect(self):
        ...
        try:
            with open(self.api_secret_path, 'r') as f:
                if "RSA" in f.readline():
                    # Using RSA V2 key
                    self.signing_scheme = signing.SCHEME_RSA_SHA256
                    self.signing_algorithm = signing.ALGORITHM_RSASSA_PKCS1v15
                else:
                    # Using EC V3 key
                    self.signing_scheme = signing.SCHEME_HS2019
                    self.signing_algorithm = signing.ALGORITHM_ECDSA_MODE_FIPS_186_3

            conf = intersight.Configuration(host=self.intersight_url,
                                            signing_info=intersight.signing.HttpSigningConfiguration(
                                                key_id=self.api_key,
                                                private_key_path=self.api_secret_path,
                                                signing_scheme=self.signing_scheme,
                                                signing_algorithm=self.signing_algorithm,
                                                signed_headers=[signing.HEADER_REQUEST_TARGET, signing.HEADER_DATE,
                                                                signing.HEADER_HOST, signing.HEADER_DIGEST, ])
                                            )
            ...
            self.api_instance = compute_api.ComputeApi(apiClient)
            print((Bcolors.HEADER +
                   '[INFO]: Configuration to Intersight is completed' +
                   Bcolors.ENDC))

        except FileHandlingException as e:
            print((Bcolors.FAIL +
                   "[ERROR]: A File Handling error occurred while parsing the Secret Key file: " +
                   Bcolors.ENDC))
            print((Bcolors.BOLD + "[ERROR-DETAIL]: " + str(e.message) + Bcolors.ENDC))
            exit()
        except Exception as err:
            print((Bcolors.FAIL +
                   "[ERROR]: An error occurred while configuring to Intersight: " +
                   Bcolors.ENDC))
    ...
    def lookup_server_by_serial(self, hostname):
        ...
        if host_serial != "<BAD INDEX>":
            ...
            if 'UCSB' in self.host_type or 'UCSX' in self.host_type:
                ...
                # Support for Colusa S-Series Servers
    ...
    def patch_server_by_moid(self, hostname, moid, os_inv_collection):
        ...
        tags = os_inv_collection + self.user_tags
        tags = {"Tags": tags}
        ...
        if 'UCSB' in self.host_type or 'UCSX' in self.host_type or 'UCS-S3260' in self.host_type or 'UCSC-C3K' in self.host_type:
            ...
            result = None
            print(Bcolors.WARNING + "[" + hostname.strip() + "]: Unknown Host type detected: " + self.host_type+ "" + Bcolors.ENDC)
            print(Bcolors.WARNING + "[" + hostname.strip() + "]: Please refer to Intersight Supported Systems: https://intersight.com/help/saas/supported_systems#intersight_essentials_features " + Bcolors.ENDC)
...
class EnvironmentConfigurator:
    ...
    def write_log(self, log_line):
        ...
        self.log_file.write(str(timestamped_log_line))
...
if __name__ == '__main__':
    try:
        ...
        print((Bcolors.FAIL +
               "[ERROR]: An error occurred while processing the given information: " +
               Bcolors.ENDC))
        print((Bcolors.BOLD + "[ERROR-DETAIL]: " + str(err) + Bcolors.ENDC))
        exit()
